...
(define (lazy-breadth-search init)
  (let search
        ...
        (let ([state  (stream-first states)]
          ...
          (stream-cons
           ...
           (search (stream-append
                    states
                    (list (multiply T1 state)
                          (multiply T2  state)
                          (multiply T3  state)
                          ))))))))

(define ppt-stream-in-tree-order
  (lazy-breadth-search '(3 4 5)))
...
(define (pairs G H)
  (let loop((first1 G) (second1 H)(cnt 0)(str (naturals-from 0)))
    (let loop2((fir first1) (sec second1)(count cnt)(st str))
       (cond
         ((<= count (stream-first st)) (stream-cons (cons  (stream-first fir) (stream-first sec))
                                                             (loop2 (stream-rest fir)  sec (+ 1 count) st) ))
         (else (loop first1 (stream-rest second1) 0 (stream-rest str))
      ))
    )))
...
(define (odd n)
  (let loop ((seed n))
    (stream-cons seed (loop (+ 2 seed)))))

(define gh-pairs-stream
 (stream-filter (λ (p) (= (gcd (car p) (cdr p) ) 1)) (pairs (odd 1) (odd 3))))
...
(define ppt-stream-in-pair-order
  (stream-map (λ (p) (list (* (car p) (cdr p) ) (/ (- (expt (cdr p) 2) (expt (car p) 2) )2)
                                                (/ (+ (expt (cdr p) 2) (expt (car p) 2) )2))) gh-pairs-stream))
